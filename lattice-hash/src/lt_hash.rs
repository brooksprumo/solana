...
impl LtHash {
    ...
    #[must_use]
    pub fn with2(hasher: &blake3::Hasher) -> Self {
    ...
    pub fn with(hasher: &blake3::Hasher) -> Self {
        ...
        let mut reader = hasher.finalize_xof();
        let x16 = [const { MaybeUninit::<u16>::uninit() }; Self::NUM_ELEMENTS];
        let mut x8 = unsafe { mem::transmute::<_, [_; Self::NUM_ELEMENTS * 2]>(x16) };
        reader.fill_uninit(&mut x8);
        Self(unsafe { mem::transmute(x8) })
    }
...
mod tests {
    ...
    fn test_hello_world2() {
        ...
        for (input, expected_lt_hash, expected_checksum) in [
            ...
            let mut hasher = blake3::Hasher::new();
            hasher.update(input.as_bytes());
            let actual_lt_hash = LtHash::with2(&hasher);
            assert_eq!(actual_lt_hash, expected_lt_hash);
            let actual_checksum = actual_lt_hash.checksum();
            assert_eq!(actual_checksum, expected_checksum);
        }
