...
impl RingDirRemover {
    ...
    pub fn with_ring(ring: IoUring) -> RingDirRemover {
        RingDirRemover {
    ...
    fn remove(&mut self, path: impl Into<PathBuf>, remove_root: bool) -> io::Result<()> {
        ...
        while let Some(dir_path) = stack.pop_front() {
            ...
            let dir_key = self.ring.ctx_mut().dirs.insert(Directory::new(fd));

            for entry in fs::read_dir(&dir_path)? {
                let entry = entry?;
                let dir = &mut self.ring.ctx_mut().dirs[dir_key];
                ...
                if is_dir {
                    ...
                    // Safety:
                    // The path pointer is guaranteed to be valid for the lifetime of UnlinkOp,
                    // which gets dropped only once it completes
                    unsafe { self.ring.push(Op::Unlink(op))? }
                }
            }
            ...
            // if the directory was empty or all unlink ops have already completed, we can close the
            // directory fd
...
impl UnlinkOp {
    fn new(dir_key: usize, dir_fd: RawFd, file_name: OsString) -> UnlinkOp {
        UnlinkOp {
            dir_key,
            dir_fd,
            path: null_terminated(file_name.into_vec()),
        }
    }
        ...
        Self: Sized,
    {
        ...
        let dir = &mut ring.ctx_mut().dirs[self.dir_key];
        ...
        if dir.scanned_and_unlinked() {
            ...
            if let Some(fd) = dir.fd.take() {
                unsafe {
                    ring.push(Op::Close(CloseOp::new(self.dir_key, fd.into_raw_fd())))?;
                }
            }
        }
...
impl CloseOp {
    ...
    fn complete(self, res: io::Result<i32>, ring: &mut RingCtx<State, Op>) -> io::Result<()> {
        let _ = res?;
        let _ = ring.ctx_mut().dirs.remove(self.dir_key);
        Ok(())
    }
}
...
impl RingOp<State> for Op {
    ...
    fn complete(self, res: std::io::Result<i32>, ring: &mut RingCtx<State, Self>) -> io::Result<()>
    where
        Self: Sized,
    {
        match self {
            Op::Unlink(op) => op.complete(res, ring),
            Op::Close(op) => op.complete(res, ring),
        }
    }
}

fn null_terminated(mut v: Vec<u8>) -> Box<[u8]> {
    v.push(0);
    v.into_boxed_slice()
}
