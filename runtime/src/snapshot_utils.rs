...
use {
    crate::{
        ...
        serde_snapshot::{
            ...
            SnapshotStreams, StartupHints, StorageListItem, StoragesList,
        },
...
pub fn mark_bank_snapshot_as_loadable(bank_snapshot_dir: impl AsRef<Path>) -> io::Result<()> {
    write_fastboot_version_to_snapshot(bank_snapshot_dir.as_ref(), &SNAPSHOT_FASTBOOT_VERSION)
}

fn write_fastboot_version_to_snapshot(
...
fn is_snapshot_fastboot_compatible(
    ...
    match version.major {
        // Current format
        4 => Ok(true),
        // Storages list lives next to the bank snapshot file.
        // Obsolete accounts use u64 file offsets. Migrate to u32 logical offsets at load time.
        3 => Ok(true),
...
fn migrate_legacy_hardlinks(bank_snapshot_dir: &Path, account_run_paths: &[PathBuf]) -> Result<()> {
    ...
    write_fastboot_version_to_snapshot(bank_snapshot_dir, &Version::new(3, 0, 0))?;
...
mod tests {
    ...
    fn test_is_snapshot_fastboot_compatible() {
        for major in 0..SNAPSHOT_FASTBOOT_VERSION.major {
            assert!(is_snapshot_fastboot_compatible(&Version::new(major, 0, 0)).is_ok());
        }
