...
impl ReadOnlyAccountsCache {
    ...
    pub(crate) fn new(max_data_size: usize) -> Self {
        ...
        let (lru_sender, lru_receiver) = crossbeam_channel::unbounded();
    ...
    pub(crate) fn load(&self, pubkey: Pubkey, slot: Slot) -> Option<AccountSharedData> {
        ...
        if selector == 1 {
            ...
            // self.queue is modified while holding a reference to the cache entry;
            // so that another thread cannot write to the same key.
            let (_, measure_send) = measure!(self.lru_sender.send(index));
            let (_, measure_stats) = measure!({
                self.account_clone_us
                ...
                self.hits.fetch_add(1, Ordering::Relaxed);
                self.update_lru_us
                    .fetch_add(measure_send.as_us(), Ordering::Relaxed);
            });
            measure_total.stop();

            const SLOW_THRESHOLD: Duration = Duration::from_millis(1);
            if measure_total.as_duration() > SLOW_THRESHOLD {
                error!("bprumo DEBUG: SLOW READ CACHE 5! load{measure_total}, get{measure_get}, clone account{measure_clone_account}, drop{measure_drop}, send{measure_send}, stats{measure_stats}");
            }
