...
pub fn accounts_db_args<'a, 'b>() -> Box<[Arg<'a, 'b>]> {
    vec![
        ...
        Arg::with_name("accounts_index_limit")
            ...
            .long_help(
                "Sets the memory limit for the accounts index. \
                 The size options will limit the accounts index memory to the specified value. \
                 E.g. \"50GB\" means the accounts index may use up to 50 GB of memory. \
                 The \"unlimited\" option keeps the entire accounts index in memory. \
                 The \"minimal\" option reduces memory usage as much as possible. \
                 All index entries that are not in memory are kept in the disk-backed index. \
                 The disk-backed index has lower performance; prefer higher limits here.",
            ),
